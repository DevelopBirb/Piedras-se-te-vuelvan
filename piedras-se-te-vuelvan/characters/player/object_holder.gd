...
@onready var holder_position := position
...
func _process(_delta: float) -> void:
	...
	if player.in_ossuary:
		follow_mouse_during_ossuary()
	else:
...
func pick_up_object(object : Node3D):
	...
	object.get_child(0).set_deferred("disabled", true)
...
func release_object_if_surface_flat(object : Node3D):
	if target_is_object() || surface_vertical_enough():
		...
		object.freeze = false
		object.get_child(0).set_deferred("disabled", false)
...
func follow_mouse_during_ossuary():
	global_position = detector_ray_cast_3d.get_collision_point() + Vector3.UP * release_height
